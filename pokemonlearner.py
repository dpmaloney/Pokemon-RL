...
def createPlayer():
    ...
    model = Sequential()
    model.add(Dense(16, activation="elu", input_shape=(1,11)))
...
if __name__ == "__main__":
    env_player_rand, model_rand, dqn_rand, policy_rand = createPlayer()
    env_player_max, model_max, dqn_max, policy_max = createPlayer()
    ...
    env_player_max.play_against(
        env_algorithm=dqn_training,
        opponent=huer,
        env_algorithm_kwargs={"dqn": dqn_max, "nb_steps": NB_TRAINING_STEPS},
    )
    model_max.save("model_%d_vsMax" % NB_TRAINING_STEPS)

    print("\nAgent Trained vs huesristic: Results against random player:")
    env_player_max.play_against(
        env_algorithm=dqn_evaluation,
        opponent=rand,
        env_algorithm_kwargs={"dqn": dqn_max, "nb_episodes": NB_EVALUATION_EPISODES},
    )
